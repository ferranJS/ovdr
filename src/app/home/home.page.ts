...
grabCircle = () => { // !!!!!!!!
  this.last().some(path => {
      ...
      if(this.overNode(path.point, path.radius)) return
  })
}
...
init = () => {
  ...
  console.log("this.c_out: ", this.c_out);
  ...
  this.btn_pintar = document.getElementById('modoPintar')
  this.btn_lineas = document.getElementById('modoLineas')
  this.btn_borrar = document.getElementById('modoBorrar')
  this.btn_grab = document.getElementById('modoGrab')
...
prepareCanvas = () => {
  ...
  this.btn_lineas.click()  // botón presionado inicial!
...
changeGrosor = (grosor: number) => { this.ctx_tmp.lineWidth = grosor || this.slider.value }

selectMode = (clickedMode: any) => {
  this.computeFrame(true)

  if(this.modo == clickedMode) this.btn_grab.click()
